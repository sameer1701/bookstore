...
SONAR_INSTANCE="${1}"
SONAR_ACCESS_TOKEN='846b2b400f2b85081a8c843d935e2bc6726e2f8c'
SLEEP_TIME=5
...
do
  ce_status=$(curl -s -u "${SONAR_ACCESS_TOKEN}": "${SONAR_INSTANCE}"/api/ce/task?id=${ce_task_id} | grep -Po '(?<="status":")[^"]+')
...
ce_analysis_id=$(curl -s -u $SONAR_ACCESS_TOKEN: $SONAR_INSTANCE/api/ce/task?id=$ce_task_id | grep -Po '(?<="analysisId":")[^"]+')
echo "QG Script --> Using analysis id of ${ce_analysis_id}"

# get the status of the quality gate for this analysisId
qg_status=$(curl -s -u $SONAR_ACCESS_TOKEN: $SONAR_INSTANCE/api/qualitygates/project_status?analysisId="${ce_analysis_id}" | grep -Po '(?<="status":")[^"]+')
